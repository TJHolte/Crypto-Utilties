...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        //while either value is zero the loop stops
        while (!n.isZero() && !m.isZero()) {
            //find the remainder of the smaller one divided into bigger one
            if (n.compareTo(m) > 0) {
                remainder = n.divide(m);
                n.copyFrom(remainder);
            } else {
                remainder = m.divide(n);
                m.copyFrom(remainder);
            }
        }
        //returning n is the goal, so copy the remaining correct answer into n
        //if it happens to be in m at the end of the loop
        if (n.isZero()) {
            n.transferFrom(m);
        }
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        //if power is zero the answer is one
        if (p.isZero()) {
            n.copyFrom(one);

            //these are almost the same except for an extra multiplication of
            //the original base value for odd

            //divide the power by two, then create a temp copy of n.
            //powerMod the temp variable with the permanently decreasing power
            //until the power is zero. powerMod uses fast powering to half
            //the exponent and multiply the base by itself however many times
            //it's needed as described by the decreasing power. Remember the
            //extra multiplication if odd.
        ...
        //this is the mod part where each recursive call has its mod by m found
        remainder = n.divide(m);
        n.copyFrom(remainder);
    }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber w2 = new NaturalNumber2();
        w2.copyFrom(w);
        //squaring itself
        w.multiply(w2);
        //modular division and comparing to one, as described in contract
        NaturalNumber remainder = w.divide(n);
        if (remainder.compareTo(one) == 0) {
            witness = true;
        }
        //new NaturalNumber needed to ensure there's always an original version
        //of w
        NaturalNumber w3 = new NaturalNumber2();
        w3.copyFrom(w2);
        NaturalNumber n2 = new NaturalNumber2();
        n2.copyFrom(n);
        //decrementing per contract
        n.decrement();
        powerMod(w2, n, n2);
        if (w2.compareTo(one) != 0) {
            witness = true;
        }
        w.copyFrom(w3);
        n.increment();
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            //this subtract four is needed to ensure nCopy is within the range
            //for the witness test
            NaturalNumber four = new NaturalNumber2(4);
            NaturalNumber nCopy = new NaturalNumber2();
            nCopy.copyFrom(n);
            nCopy.subtract(four);
            final int thirty = 30;
            //using thirty because it's between 10 and 50
            for (int i = 0; i < thirty; i++) {
                //new natural number for witness candidate is a random num
                //each time
                NaturalNumber candidate = new NaturalNumber2();
                candidate.copyFrom(randomNumber(nCopy));
                //the witness candidate has to be at least 2
                candidate.increment();
                candidate.increment();
                if (isWitnessToCompositeness(candidate, n)) {
                    //isPrime2 is here as a variable to store if
                    //any of the witness candidates ever return compositeness
                    isPrime2 = false;
                }
            }
            if (isPrime2 == false) {
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
         ...
         */
        NaturalNumber two = new NaturalNumber2(2);
        //if it's not prime, keep going
        while (!isPrime2(n)) {
            //if it's even, add one because all evens are not prime
            if (isEven(n)) {
                n.increment();
            }
            //every odd
